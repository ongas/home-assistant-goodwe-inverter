...
class Temp(Sensor):
    ...
    def __init__(
        self, id_: str, offset: int, name: str, kind: Optional[SensorKind] = None
    ):
...
class Decimal(Sensor):
    ...
    def encode_value(self, value: Any, register_value: bytes = None) -> bytes:
        return int.to_bytes(
            int(float(value) * self.scale), length=2, byteorder="big", signed=True
        )
...
class EnumBitmap22(Sensor):
    ...
    def read(self, data: ProtocolResponse):
        return decode_bitmap(
            read_bytes2(data, self.offset, 0)
            << 16 + read_bytes2(data, self._offsetL, 0),
            self._labels,
        )
...
class EcoModeV1(Sensor, EcoMode):
    ...
    def __str__(self):
        return (
            f"{self.start_h}:{self.start_m}-{self.end_h}:{self.end_m} {self.days} "
            f"{self.power}% "
    ...
    def encode_charge(self, eco_mode_power: int, eco_mode_soc: int = 100) -> bytes:
        ...
        return bytes.fromhex(
            "0000173b{:04x}ff7f".format((-1 * abs(eco_mode_power)) & (2**16 - 1))
        )
...
class Schedule(Sensor, EcoMode):
    ...
    def __init__(
        ...
        offset: int,
        name: str,
        schedule_type: ScheduleType = ScheduleType.ECO_MODE,
    ):
    ...
    def read_value(self, data: ProtocolResponse):
        self.start_h = read_byte(data)
        if (
            (self.start_h < 0 or self.start_h > 23)
            and self.start_h != 48
            and self.start_h != -1
        ):
            raise ValueError(f"{self.id_}: start_h value {self.start_h} out of range.")
    ...
    def encode_charge(self, eco_mode_power: int, eco_mode_soc: int = 100) -> bytes:
        ...
        return bytes.fromhex(
            "0000173b{:02x}7f{:04x}{:04x}{:04x}".format(
                ...
                (-1 * abs(self.schedule_type.encode_power(eco_mode_power)))
                & (2**16 - 1),
                eco_mode_soc,
    ...
    def encode_discharge(self, eco_mode_power: int) -> bytes:
        """Answer bytes representing all the time enabled discharging eco-mode group"""
        return bytes.fromhex(
            "0000173b{:02x}7f{:04x}0064{:04x}".format(
                255 - self.schedule_type,
    ...
    def encode_off(self) -> bytes:
        ...
        return bytes.fromhex(
            "30003000{:02x}00{:04x}00640000".format(
                self.schedule_type.value, self.schedule_type.encode_power(100)
            )
        )
    ...
    def set_schedule_type(self, schedule_type: ScheduleType, is745: bool):
        ...
        if schedule_type == ScheduleType.ECO_MODE:
            ...
            if self.schedule_type not in (
                ScheduleType.ECO_MODE,
                ScheduleType.ECO_MODE_745,
            ):
                self.schedule_type = (
                    ScheduleType.ECO_MODE_745 if is745 else ScheduleType.ECO_MODE
                )
        else:
